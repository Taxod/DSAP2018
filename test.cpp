...
class big_int{
    private:
        string num;
        bool negative = false;//正負號，true為負 
        void check();
        void print();//檢查cal是否正確
        big_int merge(big_int q);//合併兩個bigint，不管正負號
    public:
        int len = 0;//bigint長度
        np cal[100]={{0}};
        big_int();
...
ostream& operator<<(ostream& out,const big_int& q);
istream& operator>>(istream& in,big_int& q);
big_int find_object(string s,int nameCnt,big_int** ptr,string name[]);//找出來輸入的變數是哪一個變數
big_int calculate(string cs, big_int** ptr,string name[],int nameCnt);//計算輸入的運算式
...
int main(){
    string cs;//存放輸入的字串
    string name[20];//存放輸入的名字
    int nameCnt = 0;//計算宣告變數的數量
    big_int **BIGP = new big_int* [20];//存放宣告的BigInt
    while(getline(cin,cs)){
        ...
        if (cs.find("int") != string::npos)
        {
            ...
            BIGP[nameCnt] = new big_int(cs);//宣告BigInt
            nameCnt++;
        ...
        }else if (cs.find("cout") != string::npos)
        {
            ...
            }else if (cs.find("[") != string::npos)//找特定位數
            {
